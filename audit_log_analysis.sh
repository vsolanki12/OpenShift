 ...
 then
   ...
   for i in $KUBE_APISERVER_LOG_PATH
    do
     ...
     echo "*****************************************************"
     zcat $HOME/$CASE_ID/$CHOOSED_MUST_GATHER/$MUST_GATHER_File/$QUAY_PATH/audit_logs/kube-apiserver/$AUDIT_LOG_FILE_NAME-audit* |jq 'select (.objectRef.namespace != null)|.objectRef.namespace' | sort | uniq -c | sort -nr | head
     echo "================================================================================================================================================================"
